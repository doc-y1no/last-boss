    ...
    const fetchData = async () => {
      try {
        const promises = selectedPrefectures.map((prefCode) =>
          axios.get(`https://opendata.resas-portal.go.jp/api/v1/population/composition/perYear?cityCode=-&prefCode=${prefCode}`, {
        ...
        responses.forEach((response, index) => {
          ...
          const typeData = resultData.find((data: any) => data.label === populationType);
          typeData.data.forEach((item: any) => {
            if (item.year <= 2045) {
              if (!populationData[item.year]) {
                populationData[item.year] = { year: item.year };
              }
              populationData[item.year][`pref${selectedPrefectures[index]}`] = item.value;
            }
          });
        });
